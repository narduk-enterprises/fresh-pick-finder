...
resolve_target() {
  if [ -d .github/skills ] && [ ! -L .github/skills ]; then
    printf '%s\n' ../.github/skills
    return 0
  fi
...
ensure_links() {
  ...
  prune_skills_tree .agents/skills
  prune_skills_tree .github/skills

  for root in .agent .cursor .codex .claude; do
    mkdir -p "$root"
...
publish_skills() {
  ...
  if [ -L .github/skills ] || [ -f .github/skills ]; then
    rm -f .github/skills
  elif [ -e .github/skills ] && [ ! -d .github/skills ]; then
    rm -rf .github/skills
  fi
...
case "$MODE" in
  ...
  vendor-if-default-branch | vendor-if-upstream-head)
    ...
    ;;
  *)
    printf 'Usage: %s [links-only|publish|vendor|vendor-if-default-branch|vendor-if-upstream-head]\n' "$0" >&2
    exit 1
    ;;
esac
